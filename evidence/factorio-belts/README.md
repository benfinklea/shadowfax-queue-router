...
## Ben, 2:30 PM CDT 2026-09-12 - real numbers from the game's own Lua

Ben: "use the real inserter rotation speed and positions from the lua. for the rest of the sprites, too. Let's make this look as real as we can." Then "make the gears on the assemblers rotate when they are working" and "make the grass better - looks like you just used a few instead of rendering a nice background." Everything below is read from `/Applications/factorio.app/Contents/data/base/prototypes/entity/` on his Mac (1,847 Lua files; the engine itself is a binary and not readable).

- **One tile = 24px.** Every sprite is its hi-res sheet at 0.375 (Lua `scale = 0.5` x 24/32), so footprints are exact: assembling machine 3x3 (72px, 80x89 frame with the sheet's own overhang), steel chest 1x1 (24x30), fast inserter 1x1 (platform 39x30, hand 14x31 pivoting on the bearing at (19,11)), belt one tile wide (the frame's own 64x64 tile centre, south and north rows), rocket silo 9x9 (216px footprint; a 261px composite of shadow, hole, door back, door front, base and front at their `util.by_pixel` shifts). Items on belts at icon scale 0.5 = half a tile. A 1x1 entity carries its number just above itself, alt-mode style. Every stage reserves the sparkline slot so all machines in a row stand on one ground line.
- **Inserters swing at `rotation_speed = 0.04`** turns/tick: 180 degrees in 12.5 ticks = 0.2083s each way, hand reaching `pickup_position` 1.0 tile and `insert_position` 1.2 tiles (scaleY .77 -> .93 of the 31px hand), closed out and open back. The swing is a Web Animation per inserter (CSS keyframes cannot say "swing 0.42s then wait a variable time"): it swings at full speed, then waits at the pickup side for the rest of its period (24 / rate seconds, floored at the real 0.4167s cycle). The item in hand rides the same animation. Reduced motion: no swing, item shown still.
- **Assembler gears spin when working.** The real 32-frame `assembling-machine-3.png` sheet (214x237 @0.5, 8 per line, `animation_speed 0.5` = 30 fps): x steps the 8 columns every 0.2667s, y the 4 rows over the 1.0667s loop. Working = the stage's count > 0; gate verdicts at n/a and a machine at 0 stay still.
- **Belts run at `speed = 0.03125`** tiles/tick = 1.875 tiles/s: one 24px tile of tread per 0.5333s, always - a real belt never stops under a jam or an empty run; the state is the tint (rate now lives only in the inserters' cadence, superseding order 9's "rate is speed" for the tread).
- **Grass is a rendered field:** `grass-1.png`'s sixteen 256px variants placed at random over 4096x2048, over-stamped with its 128px (18%) and 64px (10%) variants the way the game's autoplace mixes sizes, resampled to 1536x768 at the strip's scale.

Guards (`factorio-belts-evidence.py`): footprint sizes and ratios (3 chests per assembler, 9 per silo), platform/hand pixel sizes, belt tile art, size and speed, the swing's keyframes (180 degrees, .77 -> .93 reach, drop at 0.2083s, back at 0.4167s, then hold to the period end, 4s at 6/h), item held to the drop and gone after, assembler working set and 30 fps steps with a frame actually advancing, grass period, reduced motion stopping swing, item and gears. Swing frames are captured by seeking every animation to pickup / mid-swing / drop. Mutants (run with a fresh server each): rotation speed halved, gears never spin, belt speed wrong, chest not 1x1, hand reach flat - all killed by named assertion.

## Ben, 3:27 PM CDT 2026-09-12 - one screen

Ben: "tighten everything up vertically. I'd like the full CI/CD to fit on one screen easily" (his window: 1512x850 CSS px). Row-1 gap 56 -> 14 (the row-end belts span sprite to sprite regardless); row 3 - the conflict loop and the closed-issues list, all of it right of the silo - is pulled 98px up into the bare ground beside the 9x9 silo's lower half; the legend moves up into the panel's head row beside the repo dropdown instead of taking a line; sparkline slot 26 -> 22; the closed list is denser (10 rows in ~150px, 500px wide so it clears DEPLOYED). Strip: 591px, bottom at y=787 of 850 with a full 10-row list.

Guard (`one-screen-1512x850.png`): at 1512x850 with 10 closed issues the strip ends above y=800 and is under 620px tall; the list clears DEPLOYED, every row-2 belt and the shared belt; the loop clears row 2's stage content (children rects, since row 2's flex boxes all stretch to the silo's height); the legend sits above the ground and off the dropdown and refresh button. The first two cuts of the pull (-150px, then -120px) each failed a guard - the list ran into MERGED's sparkline, then clipped a row-2 belt's bottom inserter by 13px, and the legend sat on the list's last row - which is what set the numbers above and added the belt and legend clauses. Mutants: rows back to 56px gaps - killed ("runs past a 850px window"); row 3 pulled to -190px - killed (overlap).

## Ben, 3:40-3:44 PM CDT 2026-09-12 - reach, belts up, 3x3 chests, click lists

- **"The row 2 belts are too low."** They were centred in row 2's band, which the 9x9 silo makes tall. Every in-row belt now pins to its row's ground line (stage padding + sparkline slot = 33px), level with the machines.
- **"The inserters have to be right between the belt and the item they're inserting into. If they're off, they can't reach it."** Two things followed. A station's box is now exactly its footprint wide (72px; the silo 216) with the caption, sub-lines and age in an absolutely-positioned text block hanging below it, so the inserter on either side touches the machine. And an in-row belt is exactly the machine's height, three tiles: the inserters on its top and bottom tiles are both beside the 3x3 station (a 140px belt beside a 72px machine had its bottom inserter beside nothing). Items pack at the game's 4 per tile (capacity 12), so a jam still reads as a dense line. Guard: for all eleven in-row belts, belt top and bottom within 2px of the machine's, loader/unloader gap to their machine <= 1px, both inserters level with their machine; the row-end columns touch too. Row 1 is right-aligned so IN REVIEW / GATE VERDICTS / CONFLICTED stack in one straight column; the inner frame keeps 75px on the right for that column (at 1440 wide it was getting clamped into IN REVIEW).
- **"You can make the steel boxes bigger to match the size of the assembler."** The one deliberate departure from real scale: chests draw 3x3 (72x90) like the machines, so every station's number and caption read the same. The biter nest is a 3x3 footprint too.
- **Click lists.** Ben: "when clicking on most squares, they say 'no live agents on this square' - not that helpful." That line was the lane list, which only DISPATCHED ever has. A click now shows what is inside the square: review routed / in review / conflicted / approved list their PRs (number + title, links; server adds `*_prs` rows, oldest-updated first, capped at 40, derived from the open-PR list already fetched - no extra GitHub calls), in queue / merged / deployed already did; the squares that are a count of things we do not list (issues, CI runs, commits to fold) say what the number is and link the GitHub view that lists them; gate verdicts and resolved state their n/a reason. The agents block only renders when there are agents. Guard: eight stations clicked and their contents asserted; "No live agents" never appears.
- **One screen, again:** strip 579px, bottom at y=775 of 850 with 10 closed issues (row margins to 0, row 3 at -77/+45).
