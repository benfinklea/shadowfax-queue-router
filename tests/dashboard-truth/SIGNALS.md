...
`/api/local_runners` uses the CI service installation token to paginate the repository runners endpoint, caches successes and errors for 60 seconds, and supports the existing throttled manual refresh. Pagination follows the GitHub Link next URL with per_page=100. Runners whose names start with `runs-on--` or whose labels contain `runs-on=` are excluded and counted in `excluded_cloud`. Online/busy/idle count only the remaining online runners; the expander groups them by host with per-host online/busy/idle counts and includes their names, labels, status and busy flag. Specific fleet host labels take priority, followed by host names in runner names, then the shared `fellowship-gate` label; unrecognized names remain separate. The inventory endpoint does not supply jobs today, so that field is explicitly unavailable rather than substituting repository-wide workflow runs. HTTP 403/404 produces “no permission to read runners” with no fabricated counts. Controlled available/permission captures supplement the live capture and are named as fixtures. The suite checks pagination, caching and both permission status codes.
