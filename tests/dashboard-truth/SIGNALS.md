...
`/api/local_runners` first paginates `orgs/armbrain-io/actions/runners` with the CI installation token. Both sources exclude names starting with `runs-on--` and labels containing `runs-on=`. Inventory mode exposes `source: org_inventory` and counts online/busy/idle across retained organization runners. On HTTP 403, `source: job_history` reads workflow 255384592 in armbrain-io/armbrain: the latest 100 runs plus up to 100 in-progress runs, filtered by `updated_at` in the last hour, deduplicated and prioritized by active status then update time. At most 30 runs have their jobs read (six concurrent reads, paginated at 100 jobs per page). The optional second repository is omitted to preserve request budget. A linked run-list page or more than 30 eligible runs sets `truncated` and a visible incomplete-sample warning.

Job history counts distinct assigned non-cloud runner names as `seen_last_hour`, and names with an `in_progress` job as `busy`; hosts use the runner-name prefix. Online and idle stay null, displayed as n/a globally and per host. A completed job does not establish that its runner is idle. The card shows the source and the requested permission caption. Successful and failed snapshots cache for 60 seconds; the existing throttled manual refresh still applies. Upstream failures never publish partial counts. HTTP 404 remains unavailable. Tests cover both sources, pagination, both cloud predicates, deduplication, timestamp filtering, the cap, unknown values, and cache behavior. Current evidence is in `evidence/homelab-source/`; older repository-inventory evidence records the previous behavior.
