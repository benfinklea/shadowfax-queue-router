...
| `GET /api/ci_queue` | Actions runs for `armbrain-io/armbrain`, last 48h | direct Actions REST with the identical repo/window/status scope, ±1 churn |
...
| Shipping: CI queued/running | scoped Actions runs | direct REST counts, ±1 churn |
...
| CI card: queued, running jobs/runs, active runners, orphans | scoped Actions runs and jobs | direct Actions counts; schema distinguishes unavailable from zero |
