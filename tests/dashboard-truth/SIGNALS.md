...
| Shipping: green waiting | oldest 100 open PRs, approved + mergeable, not draft, held, or in the main GraphQL merge queue; same 45-second pipeline cache | count equals list length; re-read each listed PR and queue live; DOM asserts issues open → prs open → ci q/run → green waiting → in line → merged today → last deploy (CT) literally |
